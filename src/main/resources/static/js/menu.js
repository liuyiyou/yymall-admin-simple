	...
	var menu = {
		getMenu: function(curl) {
			...
			$.getJSON(curl, function(res) {
				...
				$.each(menuList, function(index, val) {
					var item = '<li id="menu' + val.menuId + '"><a _href="' + val.url + '"><cite>' + val.menuName +
					...
					if (val.children && val.children.length) {
						//有二级子菜单
						setTimeout(function() {
							that.getSubMenu(val.menuId, val.children);
						}, 500);
					}
				});
				$('#side-nav').empty();
		...
		getSubMenu: function(id, subList) {
			...
			$.each(subList, function(idx, sub) {
				var subItem = '<li id="menu' + sub.menuId + '"><a _href="' + sub.url + '"><i class="iconfont">&#xe6a7;</i><cite>' +
					sub.menuName + '</cite></a></li>';
				subItems.push(subItem);
				if (sub.children && sub.children.length) {
					setTimeout(function() {
						that.getSubMenu(sub.menuId, sub.children);
					}, 1500);
				}
			});
